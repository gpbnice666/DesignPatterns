package com.bo.lamdba;



import com.bo.lamdba.pojo.Student;
...
//        Stream<List<Student>> students = Stream.of(Intermediate.students);
        ...
        // sorted(Comparator 排序规测) 按照比较器规则排序 产生一个新的流
        // sorted() 按照自然排序 产生一个流
...
//        });
        ...
        //  排序  ageSorted,idSorted定义排序规则
        Comparator<Student> ageSorted = Comparator.comparing(Student::getAge).reversed();
        Comparator<Student> idSorted = Comparator.comparing(Student::getId).reversed();
        // 定义排序规程顺序,先按照 ageSorted进行排序 如果age相等 在按照id 进行排序
        Comparator<Student> studentComparator = ageSorted.thenComparing(idSorted);
